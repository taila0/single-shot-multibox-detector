...
def calculate_delta(default_boxes, ground_truth):
    ...
    delta_x = (cx_gt - cx_default) / cx_witdh
    delta_y = (cy_gt - cy_default) / cx_height
...
def calculate_gt(default_boxes, delta_hat):
    ...
    cx_gt = cx_default * w_default + delta_x
    cy_gt = cy_default * h_default + delta_y
    ...
    """
    x_hat = (default_boxes[:, 0] * default_boxes[:, 2]) + delta_hat[:, 0]
    y_hat = (default_boxes[:, 1] * default_boxes[:, 3]) + delta_hat[:, 1]
